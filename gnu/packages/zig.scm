...
(define zig-0.15-libc-abi-tools
  (origin
    ...
    (uri (git-reference
          ...
          (commit "4823a8a6222fcb5cb7da5d2f8835634bd64156d6")))
    (file-name "libc-abi-tools")
    (sha256
     (base32 "1csk0i03r34ny3k6j4wf47way4bd37sdw17rky554k05xq37swbf"))))
    ...
    (arguments
     (substitute-keyword-arguments (package-arguments zig-0.14)
       ((#:phases phases '%standard-phases)
        #~(modify-phases #$phases
            (delete 'install-glibc-abilists)
            ;; TODO: These scripts are incompatible with Zig 0.15, patch them.
            ...
            #;(add-before 'check 'install-abilists
              (lambda* (#:key inputs native-inputs #:allow-other-keys)
                ...
                (with-directory-excursion "/tmp/libc-abi-tools"
                  ...
                  (for-each
                   (lambda (libc)
                     (invoke (string-append #$output "/bin/zig") "run"
                             (in-vicinity libc "consolidate.zig"))
                     (install-file
                      (in-vicinity libc "abilists")
