  ...
  ~a~{ -> ~a~}~%")
             ...
             ((package-cross-build-system-error? c)
              (let* ((package (package-error-package c))
                ...
                (leave loc (G_ "~a: build system `~a' does not support cross builds~%")
                       (package-full-name package)
                       (build-system-name system))))
             ((package-unsupported-target-error? c)
              (let* ((package (package-error-package c))
                     (loc     (package-location package)))
                (leave loc (G_ "~a: does not support target `~a'~%")
                       (package-full-name package)
    ...
    (catch 'misc-error
      (lambda ()
        (or (false-if-exception (resolve-interface `(guix scripts ,command)))
            (false-if-exception (resolve-interface
                                 `(guix extensions ,command)))
            (match (search-path (extension-directories)
                                (format #f "~a.scm" command))
              (#f (throw 'misc-error "load" "No such command: ~A" command))
              (file
               (load file)
